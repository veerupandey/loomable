"""Unified Memory.compose — conversation + user + optional knowledge (live LLM).

    memory = Memory.compose(
        conversation=ConversationMemory(store=...),
        user=UserMemory(note_store=..., auto_extract=True),
        knowledge=KnowledgeMemory(documents=[...], embedder=...),  # optional
    )
...
async def main() -> None:
    ...
    memory = Memory.compose(
        ...
        knowledge=KnowledgeMemory(
            documents=[
                "Loomable agents use Memory.compose for conversation + user notes.",
                "Workflow(memory=True) is the shared blackboard — not Agent WorkingMemory.",
            ],
            embedder=embedder,
            top_k=2,
        ),
    ...
    agent = Agent(
        ...
        user_id="alex",
        instructions="Remember the user's name and preferences. Use knowledge when relevant.",
    )
    print((await agent.arun("Hi, my name is Alex and I prefer dark mode.")).output.text())
    print((await agent.arun("What do you know about me and Loomable memory?")).output.text())
